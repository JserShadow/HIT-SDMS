  ...
  style="width: 100%">
    <el-table-column type="expand">
      <template slot-scope="props">
        <el-form label-position="left" inline class="demo-table-expand" v-for="item in props.row.scholarships" :key="item._id" style="text-align: left">
...
export default {
  ...
  methods: {
    async handleSuccess(index, row) {
      const res = await axios.post('/admin/scholarship/pass', { id: row._id });
    ...
    async handleFail(index, row) {
      const res = await axios.post('/admin/scholarship/fail', { id: row._id });
      if (res.data.message === 'ok') {
        this.$message({
          type: 'success',
          message: '操作成功'
